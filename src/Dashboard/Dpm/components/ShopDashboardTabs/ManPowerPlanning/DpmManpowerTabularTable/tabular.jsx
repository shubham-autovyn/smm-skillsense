...
import DPMRelievingOperator from './components/relieving operator/RelievingOperator';
import './tabular.css';
import { TableButton } from './tabular.style';

const DpmManpowerTabular = ({ tableData, dayWiseBtn }) => {
  ...
  const relievingDialog = (
    ...
    const payload = {
      ...
      shop: parentCategories[0] || null,
      type: parentCategories[1] || null,
      sub_category: parentCategories[2] || null,
      group_name: parentCategories[3] || null,
      line: parentCategories[4] || null,
      area: parentCategories[5] || null,
              ...
              >
                ...
                (treeFilterHierarchy === row.category
                  ...
                  row?.innerData?.map((data, innerIndex) => {
                    if (data.particulars === '+ attrition') return null;
                    return (
                      <div
                        style={{
                          display: 'contents',
                            ...
                            <div className="cell-2">
                              ...
                              {row?.area_supervisor_id && (
                                <button
                                  ...
                                  onClick={() =>
                                    handleClickSuperVisorDashboard(
                                      row.area_supervisor_id
                                    )
                                  }
                                  className="view-btn"
                  ...
                  <>
                    ...
                    <div className="cell">{row.particular}</div>
  ...
  const handleClickSuperVisorDashboard = (area_supervisor_id) => {
    window.localStorage.setItem('isSupervisorDashboard', JSON.stringify(true));
    window.localStorage.setItem('supervisorId', area_supervisor_id);
      ...
      >
        <div className="row header">
          {tableData?.particulars?.map((date, index) => {
            ...
            return (
              <div className="cell" key={index} style={style}>
                {formattedDate || ''}
              </div>
            );
          })}
              ...
              <div className="row" role="button" tabIndex={0} id={row.category}>
                ...
                (treeFilterHierarchy === row.category
                  ...
                  <>
                    ...
                    {row.values.map((value, idx) => {
                      const isGap =
                        row.particulars === 'gap' || row.particular === 'gap';

                      const valueClassName = `cell cell-value ${
                        isGap && value < 0 ? 'red-value' : ''
                      } ${isGap && value > 0 ? 'green-value' : ''}`;
...
export default DpmManpowerTabular;
