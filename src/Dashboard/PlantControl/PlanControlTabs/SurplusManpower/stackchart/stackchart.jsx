...
const VEHGraphManpower = ({
  ...
  const basicRequirement = currentItems[0]?.basicRequirement || 0;
    ...
    switch (filter) {
      case 'Only Gap':
        return [
          ...
          {
            name: 'Offline',
            data: offlineData.map(() => 0),
            color: '#97999b',
          },

          {
            name: 'Online',
            data: onlineData.map(() => 0),
            color: '#58a55c',
          },
  ...
  const surplusAnnotations = currentItems?.map((item, index) => {
    return {
      ...
      label: {
        ...
        style: {
          border:
            item?.surplus > 0 || item?.surplus === 'Only Gap' ? 'none' : '',
          color: '#FFFF',
          background: '#d83b01',
          fontSize: '8px',
        },
  ...
  const options = {
    ...
    tooltip: {
      ...
      custom: function ({ series, seriesIndex, dataPointIndex, w }) {
        ...
        // Offline section
        const offlineValue = offlineData?.[dataPointIndex] ?? 0;
        tooltipContent += `
    <div style="margin-top: 5px">
      <div style="display: flex; align-items: center; justify-content: space-between;">
        ...
        const onlineValue = onlineData?.[dataPointIndex] ?? 0;
        tooltipContent += `
        ...
        //       </div>
        //       <div>
        //         <p>${relieverData[dataPointIndex] || 0}</p>
        //       </div>
        //     </div>`;

        // Gap section

        const gapValue = gapData?.[dataPointIndex] ?? 0;
        tooltipContent += `
            <div style="display: flex; align-items: center; justify-content: space-between;">
              <div style="display: flex; align-items: center; gap: 10px;">
                <span>
        ...
        const basicReqValue = basicReqData?.[dataPointIndex] ?? 0;
        tooltipContent += `
            <div style="display: flex; align-items: center; justify-content: space-between;">
              ...
              <div>
                <p style="margin: 0;">${basicReqValue}</p>
              </div>
            </div>`;

        // Surplus section
        const surplusValue = surplusData?.[dataPointIndex] ?? 0;
