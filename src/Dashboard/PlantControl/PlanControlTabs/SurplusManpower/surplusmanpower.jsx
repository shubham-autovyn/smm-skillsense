...
import useFetchSurplusOperator from '../../../Dpm/hooks/fetchSurplusOperator';
import useManpowerSurplusGraph from '../../../Dpm/hooks/surplusGraph';
...
const SurplusManpower = ({ initialData }) => {
  ...
  }, [selectedFromDate, selectedToDate, dayWiseBtn, shop.id, treeFilter]); // Added treeFilter
  ...
  const fetchData = async () => {
    try {
      const basicPayload = {
        ...
        plantId: treeFilter?.plantId || null,
        shopId: treeFilter?.shopId || '',
        type: treeFilter?.selectedNodes?.[3] || '',
        subCategory: treeFilter?.selectedNodes?.[4] || '',
        group: treeFilter?.selectedNodes?.[5] || '',
        line: treeFilter?.selectedNodes?.[6] || '',
        area: treeFilter?.selectedNodes?.[7] || '',
  ...
  //   const oldData = filterTableData;

  //   const filteredData = oldData?.filter((val) => {
  ...
  return (
    <div className="surplus-manpower2">
      <Backdrop
        sx={(theme) => ({
          color: '#fff',
          zIndex: theme.zIndex.drawer + 1,
        })}
        open={graphLoader || surplusLoader}
      >
