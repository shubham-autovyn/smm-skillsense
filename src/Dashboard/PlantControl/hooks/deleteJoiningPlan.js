...
const useDeleteJoiningPlan = () => {
  ...
  const fetchDeleteData = useCallback(
    async (body) => {
      ...
      const params = {
        url: `smm/plant/deleteJoiningPlan?${body}`,
        config: { signal },
        body,
      };
      try {
        ...
        const response = await cachedFetch(cacheKey, () =>
          apiService.delete(params.url, params.body, params.config)
        );
...
export default useDeleteJoiningPlan;
