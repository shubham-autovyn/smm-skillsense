...
const useUpdateTrainingPlan = () => {
  ...
  const updateTrainingPlan = useCallback(
    async (body) => {
      ...
      const params = {
        url: `smm/operator/trainingPlanUpdate`,
        config: { signal },
        body,
      };
      try {
        ...
        const response = await cachedFetch(cacheKey, () =>
          apiService.put(params.url, params.body, params.config)
        );
...
export default useUpdateTrainingPlan;
