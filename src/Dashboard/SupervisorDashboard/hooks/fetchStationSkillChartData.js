...
const useFetchStationSkillChart = () => {
  ...
  const fetchStationSkillChartData = useCallback(
    async (body) => {
      setLoading(true);
      setError(null);
      ...
      const params = {
        url: `smm/supervisor/getSkillCountStationWise?${body}`,
        config: { signal },
        body,
      };
      try {
  ...
  return {
    stationSkillChartDat: cachedData,
    loading,
    error,
...
export default useFetchStationSkillChart;
