...
const useFetchAreaDashboardSkill = () => {
  ...
  const fetchAreaDashboardSkill = useCallback(
    async (body) => {
      setLoading(true);
      setError(null);
      ...
      const params = {
        url: `smm/supervisor/getSkillByStaffId?${body}`,
        config: { signal },
        body,
      };
      try {
  ...
  return {
    areaDashboardSkill: cachedData,
    loading,
    error,
    fetchAreaDashboardSkill,
    removeCache,
    cancelRequest,
  };
};
export default useFetchAreaDashboardSkill;
