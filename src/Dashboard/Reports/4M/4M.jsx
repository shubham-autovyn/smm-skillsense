...
import useSmm4M from '../hooks/smm4M';
import {} from './4M.css';
import FourMDataTable from './4MDataTable/4MDataTable';

const TabPanel = ({ value, index, children }) => {
  return value === index ? <Box>{children}</Box> : null;
};

const FourMData = ({ showFilters, setShowFilters, handleApplyFilters }) => {
  const [data, setData] = useState(0);
  const { data4M, fetch4M, loading } = useSmm4M();
  ...
  const applyFilters = async () => {
    ...
    try {
      const response = await fetch4M(payload);
    ...
    <>
      <Box className="four-m">
        ...
        <Box>
          ...
          {!showFilters && data === 1 && (
            <FourMDataTable data4M={data4M} filterSelected={filterSelected} />
          )}
        </Box>
        <Box sx={{ width: '100%' }}>
          <SnackBar
